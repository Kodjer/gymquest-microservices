...
export const updateQuest = async (req: Request, res: Response) => {
  try {
    ...
    const data = await supabaseRequest('PATCH', 'quests', updates, `?id=eq.`);
...
export const deleteQuest = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    await supabaseRequest('DELETE', 'quests', null, `?id=eq.`);
    res.status(204).send();
  } catch (error: any) {
...
export const updateQuestStatus = async (req: Request, res: Response) => {
  try {
    ...
    const data = await supabaseRequest('PATCH', 'quests', { status }, `?id=eq.`);
    res.json(data[0]);
  } catch (error: any) {
